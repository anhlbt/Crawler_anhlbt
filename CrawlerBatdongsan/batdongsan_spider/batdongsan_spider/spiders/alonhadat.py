...
from batdongsan_spider.items import AlonhadatSpiderItem
import pdb
import pandas as pd

def check_link_exists():
    try:
        bds = pd.read_csv("./alonhadat.csv", sep='|')
        link_exist = list(bds['url'].apply(lambda x: x.split("alonhadat.com.vn")[-1]))
        return link_exist
...
class AlonhadatSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		for item in items:
			...
			if item not in self.link_exist:
				item_url = "https://alonhadat.com.vn" + item
	...
	def parse_item(self, response):
		...
		item['author'] = author
		item['post_time'] = {'date': post_date.strftime("%d-%m-%Y"),'weekday': weekday}
		item['title'] = title
		item['location'] = {'province': province, 'county': county, 'road':road, 'ward': ward, 'detailed': location_detail}
		item['area'] = self.area
		item['price'] = price
